...
time is the exception. There, an animated SVG under `docs/img/` shows what
Mermaid cannot: waiting as different from running. The SVG is generated by a
script under `tools/`, not drawn by hand, so the source stays reproducible and
reviewable even though the generated file itself is binary. Which one to use
follows from what the flow needs to show, not from habit.
